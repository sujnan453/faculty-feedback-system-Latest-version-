...
const Storage = {
        ...
        try {
            ...
            ErrorHandler?.handleError(
                ErrorHandler.createError(
                    ...
                    { key, error: error.message, dataSize: JSON.stringify(value).length }
                ),
    ...
    saveDepartment(department) {
        ...
        // Check if department with same ID exists
        ...
        if (existingIndex !== -1) {
            ...
            // Check if department with same name exists (for merging duplicates)
            const duplicateIndex = departments.findIndex(d => d.name.toUpperCase() === department.name.toUpperCase());
            
            if (duplicateIndex !== -1) {
                // Merge faculties from new department into existing one
                const existingDept = departments[duplicateIndex];
                
                if (department.faculties && department.faculties.length > 0) {
                    if (!existingDept.faculties) {
                        existingDept.faculties = [];
                    }
                    
                    // Add new faculties, avoiding duplicates
                    department.faculties.forEach(newFaculty => {
                        const facultyExists = existingDept.faculties.some(
                            f => f.name.toLowerCase() === newFaculty.name.toLowerCase()
                        );
                        
                        if (!facultyExists) {
                            existingDept.faculties.push(newFaculty);
                        }
                    });
                }
                
                // Update full name if provided
                if (department.fullName && !existingDept.fullName) {
                    existingDept.fullName = department.fullName;
                }
                
                console.log(`✅ Merged department "${department.name}" with existing department. Total faculties: ${existingDept.faculties.length}`);
            } else {
...
if (Storage.getDepartments().length === 0) {
    const departments = [
        { id: Storage.generateId(), name: 'BCA', fullName: 'Bachelor of Computer Applications', faculties: [] },
        { id: Storage.generateId(), name: 'BCOM Vocational', fullName: 'Bachelor of Commerce - Vocational', faculties: [] },
        { id: Storage.generateId(), name: 'BCOM General', fullName: 'Bachelor of Commerce - General', faculties: [] },
        { id: Storage.generateId(), name: 'BSC', fullName: 'Bachelor of Science', faculties: [] },
        { id: Storage.generateId(), name: 'BA', fullName: 'Bachelor of Arts', faculties: [] }
    ];
